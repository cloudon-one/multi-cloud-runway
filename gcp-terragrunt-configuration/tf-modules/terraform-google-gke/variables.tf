...
variable "enable_resource_consumption_export" {
  type        = bool
  description = "Wheather to enable resource consumption metering on this cluster. When enabled a table will be created in the resource export BigQuery dataset to store resource consumption data. The resulting table can be joined with the resource usage table or with BigQuery billing export"
  default     = true
}
...
variable "application_secrets_key_rotation_period" {
  description = "Every time this period passes, generate a new CryptKeyVersion and set it as the primary key. The first rotation will take place after the specified period. The rotation period has the format of ta decimal numbmer with up to 9 fractional digits, followed by the letter s (seconds). It must be greater than a day (ie, 86400)"
  type        = string
  default     = null
}
